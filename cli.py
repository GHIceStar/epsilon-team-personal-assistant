...
def process_command(
    ...
    result = registry[command](CommandContext(command, book, args))
...
def run_bot(
    ...
    if help_command is not None:
        help_result = help_command(CommandContext("help", book, []))
        print_message(help_result.message, "green")

    try:
        while True:
            user_input = prompt("Enter a command: ")
            result = process_command(book, registry, user_input)
